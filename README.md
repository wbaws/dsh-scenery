...
| Default (enabled, demo setup) | Install / disabled state |
| --- | --- |
| ![preview-default](docs/preview-default.png) | ![preview-off](docs/preview-off.png) |

> Screenshots above are generated by the bundled standalone preview
> (`preview/index.html`), which renders the exact same compositing math the
> plugin uses in-app. Open it in any browser to tune every parameter live.
